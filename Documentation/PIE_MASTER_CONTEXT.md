...
* **Objective:** De-risk implementation by executing 5 focused Proof-of-Concept Spikes before core development.

```mermaid
flowchart LR
    S1[Spike 1: Azure Auth & Entra ID] --> S2[Spike 2: ADF Discovery API]
    S2 --> S3[Spike 3: Knowledge Graph Prototype]
    S3 --> S4[Spike 4: Azure AI Foundry Reasoning]
    S4 --> S5[Spike 5: Azure AI Search Indexing]
```

### Spike Breakdown:
1. **Spike 1 – Azure Authentication & RBAC Discovery:**
   * *Tooling:* Azure Identity SDK (`InteractiveBrowserCredential` / `DefaultAzureCredential`).
   * *Tasks:* Authenticate with Microsoft Entra ID; enumerate subscriptions and resource groups under Reader role.
   * *Acceptance:* Successful discovery of accessible subscriptions without requiring Contributor rights.
2. **Spike 2 – Azure Data Factory Metadata Extraction:**
   * *Tooling:* Azure Management Resource & DataFactory Client libraries.
   * *Tasks:* Enumerate Data Factories, fetch raw JSON for Pipelines, Activities, Datasets, Linked Services, Triggers, Data Flows.
   * *Acceptance:* Complete metadata ingestion across large environments (e.g. 40+ pipelines, 90+ datasets).
3. **Spike 3 – Knowledge Graph Prototype:**
   * *Tasks:* Map extracted metadata to nodes and directed edges (`USES`, `READS`, `WRITES`, `CALLS`, `DEPENDS_ON`, `TRIGGERED_BY`, `EXECUTES`).
   * *Acceptance:* Reconstruct dependency chains (e.g., `Trigger -> Pipeline -> Copy Activity -> Reads Blob -> Writes Azure SQL`).
4. **Spike 4 – Azure AI Foundry Grounded Reasoning:**
   * *Tasks:* Feed subgraph snippets into prompt templates; test for hallucination resistance.
   * *Acceptance:* Generated explanations match exact underlying metadata with zero fictitious pipeline references.
5. **Spike 5 – Azure AI Search Semantic Discovery:**
   * *Tasks:* Create search index covering asset names, descriptions, activity types, parameters, folders, and tags.
   * *Acceptance:* Querying *“Which pipelines process customer data?”* accurately ranks the relevant pipelines.

---

## 5. Phase 3: Core Platform Development (Foundation & Engine)

* **Objective:** Build the production-grade engineering foundation and non-AI portal features.

### Architectural Subsystems:
1. **Discovery Engine:** Connects to Azure Data Factory via Management REST/SDK, handles pagination, throttling, and incremental refresh.
2. **Metadata Normalization Layer:** Translates heterogeneous ADF JSON schemas into a unified, version-agnostic domain model.
3. **Domain Models:** Strict OOP/Schema models for `Pipeline`, `Activity`, `Dataset`, `LinkedService`, `Trigger`, `DataFlow`.
4. **Knowledge Graph Engine:** In-memory graph engine supporting cycle detection, ancestor/descendant traversal, and adjacency modeling.
5. **Graph Traversal Services:** Reusable core services:
   * `get_upstream_assets(asset_id)`
   * `get_downstream_assets(asset_id)`
   * `get_pipeline_dependencies(pipeline_name)`
   * `get_dataset_lineage(dataset_name)`
   * `get_linked_service_usage(linked_service_name)`
6. **Metadata Repository & Local Cache:** In-memory/JSON store for current session metadata with rapid lookup indices.
7. **Internal REST API Layer:**
   * `GET /factories`
   * `GET /pipelines`
   * `GET /pipelines/{id}`
   * `GET /datasets`
   ...
   * `GET /graph`
   * `GET /graph/{pipeline}`
8. **Asset Explorer UI:** Non-AI interactive portal allowing engineers to filter, browse, and inspect pipelines, activities, datasets, and dependency hierarchies visually.
9. **Configuration Management:** 100% environment-variable driven (Tenant ID, Subscription ID, Resource Group, Factory Name, AI Endpoints). Zero hardcoded credentials.

---

## 6. Phase 4: AI Intelligence & Engineering Experience

* **Objective:** Implement the AI reasoning layer, prompt orchestration, impact analysis, and conversational assistant.
